// This is a program system for tracking inventory levels, orders and sales and also creation of bills from sales.
	...
	int productsub[100];
	int productsadd[100];
	ofstream file;		// to read from file
	ifstream ifile;		// to write in file
	string filename;	// to store name of file
	int out;
	float itemtotal;
	...
	//function prototypes
	void displaystock(); // 4.	//Function to Display Current Stock & save Data to file.
	void defaultstock();		//Function to set default stock values.
	void items(); // 1.			//Function to manage item transactions and add Sales Made(This would Make changes in your number of units).
	void search();				//Function to search for a product.
	void exportfile();			//Function to export data to a file and Generate bill(Can only Generate a Bill for one Item purchased).
	void menu();				//Function to display the main menu.
	void addproducts();	// 2.	//Function to add products to the inventory.
	void deleteproducts(); //5.	//Function to delete products from the inventory.
	void disstock();			//Function to Display Old Stock and Display Last Data saved.
	void viewstock();			//Function to view stock details.
	void addmoreproducts();		//Function to add more products.
	...
	void displaystock()
	{
		...
		for (productid = 0; productid < 9; productid++)
		{
			...
			productsnum[productid] = productsnum[productid] + newproductsnum[productid];
			cout << productid << "\t\t\t" << products[productid] << "\t\t\t" << productsnum[productid] << "\t\t" << subproductsnum[productid] << "\t\t\t Rs" << price[productid] << endl;
		}
